    ...
    const scaleDistance = 500; // Un facteur d'échelle pour augmenter les distances apparentes
    let stars;
    ...
    function lumToSize(luminosity) {
        ...
        // modifie la taille des etoiles
        size = Math.min(Math.max(size, 50), 200);
        return size;
    }
    ...
    const createScene = (starPositions) => {
        ...
        camera.minZ = 0.1; // Minimum range
        camera.maxZ = 1000000; // Maximum range, augmentez cette valeur pour voir des objets plus éloignés
